...
def is_on_the_orb(game):
    active_orb = [orb for orb in game.orbs if orb.active]
    return active_orb
    ...
    def move(self, dx, dy, speed):
        ...
        for i in range(speed):
            ...
            if ((dy > 0 and new_y % GRID_BLOCK_SIZE == 0 or
                ...
                and (block(new_x, new_y)) or is_on_the_orb(game)):
                    return True
...
class Orb(CollideActor):
    ...
    def __init__(self, pos, dir_x):
        ...
        self.timer = -1
        self.jump_timer = self.MAX_TIMER
        self.active = False
...
class Player(GravityActor):
    ...
    def hit_test(self, other):
        ...
        if self.collidepoint(other.pos) and self.hurt_timer < 0 and isinstance(other, Bolt):
        ...
        elif self.collidepoint(other.x, other.y+15) and isinstance(other, Orb) and other.floating:
            other.active = True
            other.direction_x=0
            other.player_standing = True
            self.landed = True
            
            if other.jump_timer < 0:
                self.landed = False
                other.floating = False
                other.jump_timer = other.MAX_TIMER
                other.player_standing = False
...
class Game:
    def __init__(self, player=None, sandbox_mode=False):
        self.player = player
        self.level_colour = -1
        self.level = -1 if not sandbox_mode else 110
    ...
    def update(self):
        ...
        if state == State.PLAY:
            if len(self.pending_enemies + self.fruits + self.enemies + self.pops) == 0:
                if len([orb for orb in self.orbs if orb.trapped_enemy_type != None]) == 0 and not self.sandbox_mode:
                    self.next_level()
...
def update():
    ...
    if state == State.MENU:
        if space_pressed():
            ...
            state = State.PLAY
            game = Game(Player(), sandbox_mode=True)
        else:
            game.update()
...
def draw():
    ...
    if state == State.MENU:
        ...
        draw_text("PRESS Y FOR SANDBOX", 370, 155)
